...
export const HistorialConArbol: React.FC<HistorialConArbolProps> = ({
  ...
  let primariasList = Object.values(primariasMap).sort(
    (a, b) => new Date(b.fecha).getTime() - new Date(a.fecha).getTime()
  );

  // Búsqueda por placa (S4)
  if (searchPlaca.trim()) {
    const q = searchPlaca.trim().toLowerCase();
    primariasList = primariasList.filter((p) => {
      const pPlaca = (p.vehiculo_patente || p.vehiculo?.patente || '').toLowerCase();
      const pNum = p.numero_inspeccion.toString();
      if (pPlaca.includes(q) || pNum.includes(q)) return true;
      const subs = subregistrosMap[p.id] || [];
      return subs.some((s) => (s.vehiculo_patente || s.vehiculo?.patente || '').toLowerCase().includes(q));
    });
  }

  const canEdit = userRol === 'tecnico_inspector' || userRol === 'coordinador';
  // Ingeniero, Programador, Administrador e Inspector pueden descargar PDF individual
  const canPDF = true;

  return (
    <div className="space-y-4">
      {/* Buscador de Historial por Placa */}
      <div className="max-w-xs">
        <Input
          placeholder="Buscar por placa o N° planilla..."
          value={searchPlaca}
          onChange={(e) => setSearchPlaca(e.target.value)}
        />
      </div>

      {primariasList.length === 0 ? (
            ...
            <table className="table-industrial">
              ...
              <tbody>
                {primariasList.map((primaria) => {
                  ...
                  // La aprobación de un registro primario se basa en el último registro/subregistro hecho (S6)
                  const ultimoRegistro = hasSubs ? subs[subs.length - 1] : primaria;
                  const dictamenFinal = ultimoRegistro.resultado_general;
                  const estadoFinal = ultimoRegistro.estado;
                  const esHallazgo = dictamenFinal === 'con_hallazgos' || dictamenFinal === 'no_apto';
